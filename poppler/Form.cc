...
static int check_bom(const char *data, size_t size) {
...
static GooString *convertToUTF16v2(GooString *data) {
    ...
    int bom = check_bom(data->getCString(), data->getLength());

    if (bom == -1) {
        return convertToUtf16(data->copy());
    }
...
FormField::FormField(PDFDoc *docA, Object *aobj, const Ref &aref, FormField *parentA, std::set<int> *usedParents,
                     FormFieldType ty) {
    doc = docA;
    xref = doc->getXRef();
    aobj->copy(&obj);
        ...
        if (parent.dictLookup("T", &obj2)->isString()) {
            ...
            if (unicode_encoded) {
                ...
                if (parent_name->hasUnicodeMarker()) {
                    full_name->insert(0, parent_name->getCString() + 2,
                                      parent_name->getLength() - 2); // Remove the unicode BOM
                } else {
                    int tmp_length;
    ...
    if (partialName) {
        if (unicode_encoded) {
            if (partialName->hasUnicodeMarker()) {
                full_name->append(partialName->getCString() + 2,
                                  partialName->getLength() - 2); // Remove the unicode BOM
            } else {
                int tmp_length;
...
GBool FormFieldButton::getState(char *state) {
    ...
    return (parent && parent->getType() == formButton) ? static_cast<FormFieldButton *>(parent)->getState(state)
                                                       : gFalse;
}
...
void FormFieldSignature::parseInfo() {
    ...
    if (time_of_signing.isString()) {
        ...
        signature_info->setSigningTime(
                dateStringToTime(time_str)); // Put this information directly in SignatureInfo object
...
FormField *
Form::createFieldFromDict(Object *obj, PDFDoc *docA, const Ref &pref, FormField *parent, std::set<int> *usedParents) {
    Object obj2;
    FormField *field;
